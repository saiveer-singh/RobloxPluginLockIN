...
export function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  const { settings, updateSettings, resetToDefaults, themeColors } = useSettings();
            ...
            {activeTab === 'appearance' && (
              <div className="space-y-6">
                <div>
                  ...
                  <h3 className="text-lg font-semibold text-foreground mb-4">Customize Colors</h3>
                  <div className="space-y-3 mb-6">
                    {Object.keys(themePresets.dark).map((key) => {
                       const colorKey = key as keyof ThemeColors;
                       // Resolve the color: check customColors first, then fallback to current theme preset
                       const currentColor = settings.customColors?.[colorKey] || themePresets[settings.theme][colorKey];
                       const isCustomized = settings.customColors?.[colorKey] !== undefined;

                       return (
                        <div key={key} className="flex items-center justify-between p-2 rounded hover:bg-hover transition-colors">
                          <div className="flex items-center gap-2">
                            <label className="text-sm font-medium text-foreground capitalize w-24">
                              {key}
                            </label>
                            {isCustomized && (
                          ...
                          <div className="flex items-center gap-2">
                            <div className="relative group">
                              <input
                                ...
                                className="w-8 h-8 rounded cursor-pointer bg-transparent border-0 p-0 opacity-0 absolute inset-0 z-10"
                              />
                               <div
                             ...
                             <input
                                ...
                                className="w-24 px-2 py-1 bg-input border border-border rounded text-xs text-foreground font-mono"
                              />
                            {isCustomized && (
                              <button
                                onClick={() => {
                                ...
                                className="p-1 hover:bg-hover rounded text-secondary hover:text-error transition-colors"
